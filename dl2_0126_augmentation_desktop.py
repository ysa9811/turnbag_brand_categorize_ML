...
def train_model():
    ...
    def augment_data(image, label):
        ...
        image = tf.image.random_contrast(image, 0.7, 1.3)  # 범위를 조금 확장
        image = tf.image.random_hue(image, 0.2)  # 색조 범위 확장
        image = tf.image.random_saturation(image, 0.7, 1.3)  # 채도를 더 다양하게
        image = tf.image.random_crop(image, size=(100, 100, 3))  # 무작위로 자르기
        image = tf.image.resize(image, (128, 128))  # 다시 원래 크기로 조정
        return image, label
    ...
    #     initial_learning_rate=0.0005 , first_decay_steps=10, t_mul=2.0, m_mul=0.9
    # )
    ...
    callbacks = [
        ...
        ModelCheckpoint('0126_best_model.keras', save_best_only=True, verbose=0),
